...
function getDevSiteForIpPath(urlPath, sites) {
  ...
  const site = sites.find((entry) => entry.isDevelop && entry.name === candidate);
  if (!site) {
    return null;
  }
  const remainingPath = `/${segments.slice(1).join("/")}`;
  return { site, remainingPath: remainingPath === "/" ? "/" : remainingPath };
}
...
function handleRequest(sites) {
  return (request, response) => {
    ...
    let site = getSiteForHost(hostname, sites);
    let requestPath = request.url || "/";

    if (!site && isIpHost(hostname) && port === "80") {
      const devMatch = getDevSiteForIpPath(requestPath, sites);
      if (devMatch) {
        site = devMatch.site;
        requestPath = devMatch.remainingPath;
      }
    }
    ...
    let filePath = resolveFilePath(site, requestPath);
